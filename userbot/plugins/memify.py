...
async def _(event):
    ...
    if reply_message.sender.bot:
       ...
       await event.edit("```Please wait! Memifying this image!```")
    ...
    async with borg.conversation("@MemeAutobot") as bot_conv:
          ...
          if "Okay..." in response.text:
            await event.edit("```🤨This is not an image! This will take some time to convert it into image!```")
            thumb = None
            ...
            await event.delete()
            sax = await borg.send_message(event.chat_id, "`By @shahanurrhmn🔥🔥`")
